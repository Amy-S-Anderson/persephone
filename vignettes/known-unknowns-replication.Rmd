---
title: "Known Unknowns: Replicating the Main Analysis"
...
This vignette replicates the main analysis from "Known unknowns and the osteological paradox: Why bioarchaeology needs agent-based models" (Anderson and DeWitte) using the `persephone` package.
...
1. **Risk-free** (RMR = 1): skeletal lesions have no association with mortality risk.
2. **Risk-doubled** (RMR = 2): skeletal lesions are associated with double the baseline mortality risk.
...
survival_data1 <- run_survival_analysis(
  sim_data %>% filter(relative_risk == 1))
survival_data2 <- run_survival_analysis(
  sim_data %>% filter(relative_risk == 2))

# Adults only (ages 10+)
survival_adults1 <- run_survival_analysis(
  sim_data %>% filter(relative_risk == 1, age >= 10))
survival_adults2 <- run_survival_analysis(
  sim_data %>% filter(relative_risk == 2, age >= 10))
```
